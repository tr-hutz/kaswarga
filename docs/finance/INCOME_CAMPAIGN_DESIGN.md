...
### In scope
- `income_campaigns` table with status lifecycle, target, deadline, and Campaign Code.
- `contribution_code` field on `income_transactions` for campaign-linked monetary contributions (copied from `campaign_code` at insert time).
...
| Campaign Code | Kode Transfer Kampanye | Per-campaign bank transfer reference code entered by ALL donors of the same campaign (e.g. `MAROENS8AGT26`). Set once at campaign creation; never changes. |
| Contribution Code | Kode Kontribusi | The bank transfer reference on a specific `income_transaction` — copied directly from the campaign's `campaign_code` at insert time. All donors of the same campaign share the same value. |
...
CREATE TABLE income_campaigns (
    ...
    campaign_code  text        NOT NULL,
        -- Human-readable bank transfer reference code; 3-20 chars; RT-unique across all campaigns
        -- including deleted. Example: MAROENS8AGT26 (RT code + campaign abbreviation + year).
        -- All donors of this campaign write this same code in their bank transfer remark.
    description    text,
    ...
    CONSTRAINT income_campaigns_code_rt_unique
        ...
        -- Spans ALL campaigns including deleted: a code retires permanently
...
ALTER TABLE income_transactions
    ...
    ADD COLUMN contribution_code     text,
        -- nullable; only set for campaign-linked DONATION transactions;
        -- copied from income_campaigns.campaign_code at insert time (same value for all donors)
...
- `contribution_code` is nullable — only set for `income_category = 'DONATION'` with `campaign_id` set. Copied directly from `income_campaigns.campaign_code` at insert time; **never editable after creation** (see §4.6). Because all donors of the same campaign share the same code, there is no uniqueness constraint on this column.
...
rt
 ...
 │       id (uuid, PK)
 │       rt_id          FK → rt.id
 │       campaign_code  (text, RT-unique; bank transfer reference for all donors)
 ...
 └── income_transactions  (existing table, extended)
         ...
         contribution_code   nullable text; = campaign_code when set (same for all donors)
...
Migration: **`035_income_campaigns.sql`**
...
2. Creates `income_campaigns` table (with `campaign_code` column).
3. Alters `income_transactions` (adds `campaign_id`, `contribution_code`, `in_kind_*` columns, FK, index).
4. Alters `rt` table — adds `maker_checker_enabled boolean NOT NULL DEFAULT TRUE`.
5. Enables RLS on `income_campaigns` and creates policies (see §4.5).
...
### 4.6 Campaign Code and Contribution Code

**Campaign Code (`campaign_code`)** is a per-campaign bank transfer reference code set once at campaign creation. All donors of the same campaign use the same code as the bank transfer remark. It is a human-readable, concise string derived by Treasurer intuition from the RT name and campaign name:

```
Examples:
  MAROENS8AGT26    ← RT "MAROENS8" + campaign "Agustusan 2026"
  MAROENS8BENCANA  ← RT "MAROENS8" + campaign "Sumbangan Bencana Alam NTT"
```

**Properties:**

- Set by the creating user at campaign creation; stored in `income_campaigns.campaign_code`.
- 3–20 characters; uppercased and whitespace-stripped on save.
- RT-unique across all campaigns including deleted. A code that was ever used for any campaign in the RT cannot be reused.
- **Locked after creation** — the field cannot be changed once the campaign record exists.
...
When a monetary (`DONATION`) contribution is inserted for a campaign, the server copies `campaign.campaign_code` directly into `income_transactions.contribution_code`. There is no sequence generation, no sequential suffix, and no uniqueness constraint — all donors of the same campaign share the same `contribution_code` value.

**Donation tracking/verification without a unique code:**

The Treasurer identifies a specific bank transfer by matching **code + payer name + amount + date** against the `income_transactions` list. Since most donors of a campaign have different names and amounts, this triple match is sufficient for manual reconciliation without requiring a per-donation unique code.
...
Server validates:
- `campaign_code`: 3–20 characters; uppercased and whitespace-stripped.
- Code must not exist in the RT, including soft-deleted or cancelled campaigns (code retires permanently).

Permission: `income.campaign.create`. Activity log: `campaign_create`.

**Post-create notification:** All active ADMIN and TREASURER members receive a notification (`type: campaign_pending`) prompting them to review the campaign, so Treasurer can prepare before activation. RT_CHAIR receives a separate notification to activate.
...
Updates `name`, `description`, `target_amount`, `starts_at`, `ends_at` for `DRAFT` or `ACTIVE` campaigns. `campaign_code` is locked after creation and cannot be updated via this endpoint. Permission: `income.campaign.update`. Activity log: `campaign_update`.
...
#### `POST /api/income/campaigns/[id]/donate`
...
Server:
1. Validates campaign exists, belongs to the resident's RT, status = `ACTIVE`, `starts_at <= today`, `ends_at IS NULL OR ends_at >= today`.
2. Inserts `income_transaction` with `income_category = 'DONATION'`, `status = 'pending'`, `contribution_code = campaign.campaign_code`.
...
Server then:
1. Copies `campaign.campaign_code` into `contribution_code` on the new `income_transaction`.
...
- The server **always** sets `contribution_code` by copying `campaign.campaign_code`. There is no API parameter for the client to supply one.
...
```
Resident records contribution → contribution_code (= campaign_code) assigned
         ↓
...
**Multiple donors, same code:** Because all donors of a campaign share the same `campaign_code`, the Treasurer uses the combination of code + donor name + amount + date to disambiguate between pending records. This is sufficient for manual reconciliation at typical RT scale.
...
`DataTable` with columns: Name, Status badge, Target, Approved Progress, Period, Actions. Edit is an inline row action (gated by `income.campaign.update`).
...
- Kode Transfer (required, 3–20 chars, uppercased; locked after campaign is created — shown as disabled in edit mode)
...
Resident fills: amount, method, notes (optional), date
         ↓
Resident submits → POST /api/income/campaigns/[id]/donate
...
Server:
  ...
  3. Sets contribution_code = campaign.campaign_code (same code for all donors)
...
If approved:
  ...
  - if target reached → campaign → COMPLETED + notification to ADMIN and CHAIR
...
| `campaign_code` | Identifies the bank transfer code, not necessarily the campaign name — less human-friendly for import. |
...
- For each monetary (`DONATION`) row that resolves to a campaign, the server copies `campaign.campaign_code` into `contribution_code` at import-processing time — same as the live form.
...
|---|---|---|
| Campaign created (DRAFT) | All active ADMIN + TREASURER | `campaign_activated` (pending review) |
| Campaign activated (DRAFT → ACTIVE) | All active ADMIN + TREASURER | `campaign_activated` |
...
The `contribution_code` / `campaign_code` is designed as a **first-pass reconciliation filter** for matching bank statement lines to pending contributions. It is **not** an automatic approval trigger, and it is not unique per-donor.
...
Extract transfer remark/description
         ↓
Search for campaign_code pattern (e.g. MAROENS8AGT26) in bank remark
         ↓
Candidate set: all pending contributions for that campaign
         ↓
...
> The `campaign_code` is a strong campaign-level filter, but per-donor disambiguation is manual. Final reconciliation policy will be defined by the future banking/reconciliation architecture. This feature does not commit to any automatic approval behavior.

Design choices that support this future without coupling to it:
- `campaign_code` is RT-unique and persists even after `COMPLETED` or `CANCELLED` — valid reconciliation filter for all time.
- All `contribution_code` values for a campaign equal `campaign_code` — bank remark search is straightforward.
- Code uniqueness spans deleted campaigns: no historical ambiguity across the RT's lifetime.
...
| 6 | Campaign Code (`campaign_code`) | Per-campaign bank transfer reference; human-set at creation; 3–20 chars; RT-unique. All donors of a campaign use the same code. No sequential suffix. |
| 7 | Contribution Code assignment | Server copies `campaign.campaign_code` into `income_transactions.contribution_code` at insert. No sequence generation. Not unique per-donor. |
...
- **Campaign code assignment**: `contribution_code = campaign.campaign_code` copied at insert time. No sequence infrastructure needed.
- **Migration number**: `035_income_campaigns.sql` — confirmed against repository.
...
| `035_income_campaigns.sql` migration | ✅ Implemented |
| Beranda/Dasbor campaign card slot | ✅ Implemented |
