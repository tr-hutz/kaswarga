    ...
    const cols: Column<MappedExpense>[] = [
        {
            ...
            render: (row) => (
                <span className="font-mono text-xs text-dark-5">{row.receiptNumber || '—'}</span>
            ),
        },
        {
            key:   'dateLabel',
                ...
                return (
                    <div className="flex justify-end gap-2">
                        <button
                            ...
                            className="text-sm border border-stroke px-3 py-1 rounded-lg"
                        >
                            {tc('actions.edit')}
                        </button>
                        <button
                            onClick={(e) => { e.stopPropagation(); onDelete(row) }}
                            className="text-sm border border-stroke px-3 py-1 rounded-lg text-danger"
                        >
                            {tc('actions.delete')}
                        </button>
                    </div>
